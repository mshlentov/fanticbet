...
- [ ] 🟢 Фронт: убрать чип «Завершённые» (settled) из `STATUS_FILTERS` на ленте (`EventsPage.tsx:13-18`), оставить «Все» / «Предстоящие» / «Live». Проверить, что «Все» теперь не показывает архив.
